import{u as v,a as y}from"./Biy0voFX.js";import{O as b,o as w,i as a,m as q}from"./BNyvA7k1.js";const g=()=>{const s=v(),l=y(),d=a([]),u=a(null),o=a([]),r=a([]),n=a(!1),f=q(()=>r.value.reduce((e,t)=>e+t.event_price*t.quantity,0)),m=async()=>{if(!l.profile?.id)return;const{data:e,error:t}=await s.from("Exhibition_Booths").select("id, booth_number, exhibitions:exhibition_id (name)").eq("owner_id",l.profile.id);t||(d.value=e?.map(i=>({id:i.id,exhibition_name:`${i.exhibitions.name} (${i.booth_number})`}))||[])};b(u,async e=>{if(!e){o.value=[];return}n.value=!0;try{const{data:t,error:i}=await s.from("Exhibition_Product_Details").select(`
          id, 
          event_price, 
          is_paid,
